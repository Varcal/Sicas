(function () {
    ...
    angular.module('app').controller('sindicanteEditController', sindicanteEditController);

    sindicanteEditController.$inject = ['$routeParams','$location', 'enderecoService', 'sindicanteService', 'bancoService', 'servicoSeguradoraService', 'servicoSindicanteService'];

    function sindicanteEditController($routeParams, $location, enderecoService, sindicanteService, bancoService, servicoSeguradoraService, servicoSindicanteService) {
        var vm = this;
        ...
        function salvar() {
            vm.loading = true;
            sindicanteService.editar(vm.sindicante)
                .success(success)
                .catch(fail);
            ...
            function success(response) {
                if (response) {
                    ...
                    toastr.success("Sindicante alterado com sucesso", "SUCESSO");
        ...
        function obterPorId() {
            sindicanteService.obterPorId(id)
                .success(success)
                .catch(fail);

            function success(response) {
                vm.sindicante = response;
                pesquisarEnderecoPorCep();
            }

            function fail(error) {
                toastr.error("Não foi possível obter o sindicante", "ERRO");
                console.log(error);
            }
        }
